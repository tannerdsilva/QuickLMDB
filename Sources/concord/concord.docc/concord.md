...
- ``ConcordIndex`` — the store contract. A sorted, fixed-size-key view over one strict database: streaming key walks, range fingerprints (24-byte blake2s over mmap key bytes), and the byte-passthrough pair ``loadBytes`` (a borrowed view out) / ``storeBytes`` (a verbatim write in). `Value` is a phantom type parameter that pins the schema; it is never instantiated.
...
- **out of the store** — ``loadBytes(_:)`` returns a borrowed view over the value's own mmap page (zero copies).
- **into the store** — ``storeBytes(_:_:)`` writes the incoming bytes verbatim (one copy in).
