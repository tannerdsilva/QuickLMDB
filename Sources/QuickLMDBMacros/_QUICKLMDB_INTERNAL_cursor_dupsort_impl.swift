...
internal struct _QUICKLMDB_INTERNAL_cursor_dupsort_impl:MemberMacro {
	static func expansion(of node:SwiftSyntax.AttributeSyntax, providingMembersOf declaration:some SwiftSyntax.DeclGroupSyntax, in context:some SwiftSyntaxMacros.MacroExpansionContext) throws -> [SwiftSyntax.DeclSyntax] {
		return [
			DeclSyntax("""
				@available(*, noasync)
				public borrowing func opGetBoth(returning _:MDB_cursor_dbtype.MDB_db_val_type.Type, key keyVal:consuming MDB_cursor_dbtype.MDB_db_key_type, value valueVal:consuming MDB_cursor_dbtype.MDB_db_val_type) throws(LMDBError) -> MDB_cursor_dbtype.MDB_db_val_type {
					return try keyVal.MDB_access({ (keyMDBVal:consuming MDB_val) throws(LMDBError) in
						return try valueVal.MDB_access({ (valueMDBVal:consuming MDB_val) throws(LMDBError) in
							return try opGetBoth(transforming:MDB_val.self, valueOutTransformer: { MDB_cursor_dbtype.MDB_db_val_type($0)! }, key:keyMDBVal, value:valueMDBVal)
						})
					})
				}
			"""),
			DeclSyntax("""
				@available(*, noasync)
				public borrowing func opGetBothRange(returning _:MDB_cursor_dbtype.MDB_db_val_type.Type, key keyVal:consuming MDB_cursor_dbtype.MDB_db_key_type, value valueVal:consuming MDB_cursor_dbtype.MDB_db_val_type) throws(LMDBError) -> MDB_cursor_dbtype.MDB_db_val_type {
					return try keyVal.MDB_access({ (keyMDBVal:consuming MDB_val) throws(LMDBError) in
						return try valueVal.MDB_access({ (valueMDBVal:consuming MDB_val) throws(LMDBError)  in
							return try opGetBothRange(transforming:MDB_val.self, valueOutTransformer: { MDB_cursor_dbtype.MDB_db_val_type($0)! }, key:keyMDBVal, value:valueMDBVal)
						})
					})
				}
			"""),
