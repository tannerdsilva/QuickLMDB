...
internal struct _QUICKLMDB_INTERNAL_cursor_encodable_impl:MemberMacro {
	static func expansion(of node:SwiftSyntax.AttributeSyntax, providingMembersOf declaration:some SwiftSyntax.DeclGroupSyntax, in context:some SwiftSyntaxMacros.MacroExpansionContext) throws -> [SwiftSyntax.DeclSyntax] {
		return [
			...
			"""),
			DeclSyntax("""
				public borrowing func opSetRange(key:MDB_cursor_dbtype.MDB_db_key_type) throws {
					try key.MDB_access { (keyVal:consuming MDB_val) in
						try opSetRange(key:keyVal)
					}
				}
			"""),
			DeclSyntax("""
				public borrowing func opSet(key: MDB_cursor_dbtype.MDB_db_key_type) throws {
					try key.MDB_access({ (mdbKey:consuming MDB_val) in
						try opSet(key:mdbKey)
					})
				}
			"""),
			DeclSyntax("""
			...
			"""),
			DeclSyntax("""
				public borrowing func opGetBoth(key:borrowing MDB_cursor_dbtype.MDB_db_key_type, value:consuming MDB_cursor_dbtype.MDB_db_val_type) throws {
					try key.MDB_access { (mdbKey:consuming MDB_val) in
						try value.MDB_access { (mdbVal:consuming MDB_val) in
							try opGetBoth(key:mdbKey, value:mdbVal)
						}
					}
				}
			"""),
			DeclSyntax("""
				public borrowing func opGetBothRange(key:borrowing MDB_cursor_dbtype.MDB_db_key_type, value:consuming MDB_cursor_dbtype.MDB_db_val_type) throws {
					try key.MDB_access { (mdbKey:consuming MDB_val) in
						try value.MDB_access { (mdbVal:consuming MDB_val) in
							try opGetBothRange(key:mdbKey, value:mdbVal)
						}
					}
				}
			"""),
