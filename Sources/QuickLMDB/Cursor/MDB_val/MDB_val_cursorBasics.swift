...
extension MDB_cursor_dupsort {
	...
	public borrowing func dupCount() throws(LMDBError) -> Int {
		return try MDB_cursor_get_dupcount_static(cursor:self.cursorHandle())
	}
...
extension MDB_cursor {
	...
	public borrowing func setEntry(key:consuming MDB_val, value:consuming MDB_val, flags:consuming Operation.Flags) throws(LMDBError) {
		return try MDB_cursor_set_entry_static(cursor:self.cursorHandle(), key:&key, value:&value, flags:flags.rawValue)
	}
	
	/// get the current entry from the database
	@available(*, noasync)
	public borrowing func containsEntry(key:consuming MDB_val, value:consuming MDB_val) throws(LMDBError) -> Bool {
		return try MDB_cursor_contains_entry_static(cursor:self.cursorHandle(), key:&key, value:&value)
	}
	...
	public borrowing func containsEntry(key:consuming MDB_val) throws(LMDBError) -> Bool {
		return try MDB_cursor_contains_entry_static(cursor:self.cursorHandle(), key:&key)
	}
	...
	public borrowing func deleteCurrentEntry(flags:consuming Operation.Flags) throws(LMDBError) {
		return try MDB_cursor_delete_current_entry_static(cursor:self.cursorHandle(), flags:flags.rawValue)
	}
	...
	public borrowing func compareEntryKeys(_ dataL:consuming MDB_val, _ dataR:consuming MDB_val) -> Int32 {
		return MDB_cursor_compare_keys_static(tx:self.txHandle(), db:self.dbHandle(), lhs:&dataL, rhs:&dataR)
	}
	...
	public borrowing func compareEntryValues(_ dataL:consuming MDB_val, _ dataR:consuming MDB_val) -> Int32 {
		return MDB_cursor_compare_values_static(tx:self.txHandle(), db:self.dbHandle(), lhs:&dataL, rhs:&dataR)
	}
}