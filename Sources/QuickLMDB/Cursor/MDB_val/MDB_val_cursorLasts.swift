...
extension MDB_cursor {
	...
	public borrowing func opLast(returning:(key:MDB_val, value:MDB_val).Type) throws(LMDBError) -> (key:MDB_val, value:MDB_val) {
		...
		let entry = try MDB_cursor_get_entry(cursor:self.cursorHandle(), op:Operation.last.mdbValue, key:keyVal, value:valueVal)
...
extension MDB_cursor_dupsort {
	...
	public borrowing func opLastDup(returning:MDB_val.Type) throws(LMDBError) -> MDB_val {
		...
		let entry = try MDB_cursor_get_entry(cursor:self.cursorHandle(), op:Operation.lastDup.mdbValue, key:keyVal, value:valueVal)
