...
//
//   every environment is its own TYPE. @MDB_environment types carry their
...
public struct ClubCalendar: Sendable {
	...
	public func eventOn(_ day: DayKey) throws -> EventID? {
		#load(ClubCalendar.self, database: \.events, key: day)
	}

	@MDB_transact(.readOnly)
	public func inviteesFor(_ event: EventID) throws -> [ContactID] {
		var result: [ContactID] = []
		#cursor(ClubCalendar.self, database: \.invitees) { cursor in
	...
	public func addInvitee(_ event: EventID, _ contact: ContactID) throws {
		try #store(ClubCalendar.self, database: \.invitees, key: event, value: contact)
	}
	...
	// - MARK: multi-environment (the other environment is a TYPED parameter)

	@MDB_transact(.readWrite)
	public func scheduleAndMarkSync(_ event: EventID, on day: DayKey, contact: ContactID, at timestamp: Timestamp, contacts: ClubContacts) throws {
		try #store(ClubCalendar.self, database: \.events, key: day, value: event)
...
struct NewDialectDemo {
	...
	@Test func multiEnvBoundarySpansBothEnvironments() throws {
		let root = FileManager.default.temporaryDirectory.appendingPathComponent("qlmdb-club-multi-\(UUID().uuidString)", isDirectory: true)
		let calendar = try ClubCalendar.open(at: root.appendingPathComponent("calendar").path)
		let contacts = try ClubContacts.open(at: root.appendingPathComponent("contacts").path)
	...
	@Test func multiEnvThrowAbortsBothEnvironments() throws {
		let root = FileManager.default.temporaryDirectory.appendingPathComponent("qlmdb-club-multi-\(UUID().uuidString)", isDirectory: true)
		let calendar = try ClubCalendar.open(at: root.appendingPathComponent("calendar").path)
		let contacts = try ClubContacts.open(at: root.appendingPathComponent("contacts").path)
